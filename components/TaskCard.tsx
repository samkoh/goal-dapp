import { Card, Flex, useToast, Text } from "@chakra-ui/react";
...
export default function Task({ taskId, task, isCompleted }: Props) {
    ...
    return (
        <Card p={8} backgroundColor={backgroundColor} border={"1px solid"} borderColor={borderColor}>
            <Flex flexDirection={"row"} justifyContent={"space-between"} alignItems={"center"}>
                <Text fontWeight={"bold"} color={fontColor} textAlign={"left"}>{task}</Text>
                {isCompleted ? (
